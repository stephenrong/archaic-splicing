...
final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_introgr_hub_spliceai_gs <- final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_introgr_hub_spliceai_gs %>% 
	mutate(spliceai_sig = (spliceai_max>=0.01)&(spliceai_max<0.1))
	...
	if (min(stat_test_pair$p) < 0.05) {
		...
		p <- ggbarplot(spliceai_proportion_tab %>% arrange(spliceai_prop), 
			...
			stat_pvalue_manual(stat_test_pair_sig, label = "p", 
				tip.length = 0.008, label.size=3) + 
			theme(legend.position = "none") + ylab("Proportion SpliceAI 0.01-0.2") + 
...
output_file <- "../../results/enrichment_splice_prediction_gw_raw/Neanderthal_updated_spliceai_prop_bar_plot_stat_weak-lineage_specific_nodeni.pdf"
spacer <- 0.008
scale <- 0.55
width <- 0.2
...
output_file <- "../../results/enrichment_splice_prediction_gw_raw/Neanderthal_updated_spliceai_prop_bar_plot_stat_weak-lineage_specific_nodeni-range.pdf"
spacer <- 0.0003
scale <- 0.8  # 0.55
width <- 0.2
enrichment_spliceai_figures(list_cols, list_names, output_file, spacer, width) + 
...
output_file <- "../../results/enrichment_splice_prediction_gw_raw/Neanderthal_updated_spliceai_prop_bar_plot_stat_weak-introgressed.pdf"
spacer <- 0.008
scale <- 0.65
width <- 0.2
enrichment_spliceai_figures(list_cols, list_names, output_file, spacer, width)
