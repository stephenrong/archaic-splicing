...
final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_introgr_hub_spliceai_gs <- final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_introgr_hub_spliceai_gs %>% 
	mutate(spliceai_sig = (spliceai_max>=0.2)&(spliceai_max<0.5))
...
enrichment_spliceai_figures <- function(list_cols, list_names, output_file, spacer, width) {
	...
	if (min(stat_test_pair$p) < 0.05) {
		...
		p <- ggbarplot(spliceai_proportion_tab %>% arrange(spliceai_prop), 
			...
			theme(legend.position = "none") + ylab("Proportion SpliceAI 0.2-0.5") + 
...
output_file <- "../../results/enrichment_splice_prediction_gw_raw/Neanderthal_updated_spliceai_prop_bar_plot_stat_moderate-lineage_specific_nodeni.pdf"
spacer <- 0.001
scale <- 0.55
width <- 0.2
...
output_file <- "../../results/enrichment_splice_prediction_gw_raw/Neanderthal_updated_spliceai_prop_bar_plot_stat_moderate-lineage_specific_nodeni-range.pdf"
spacer <- 0.0003
scale <- 0.8  # 0.55
width <- 0.2
enrichment_spliceai_figures(list_cols, list_names, output_file, spacer, width) + 
...
output_file <- "../../results/enrichment_splice_prediction_gw_raw/Neanderthal_updated_spliceai_prop_bar_plot_stat_moderate-introgressed.pdf"
spacer <- 0.001
scale <- 0.65
width <- 0.2
enrichment_spliceai_figures(list_cols, list_names, output_file, spacer, width)
