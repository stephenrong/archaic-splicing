...
adaptive_enrichments_list[["weak"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/adaptive_spliceai_weak/adaptive_spliceai_weak_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_SpliceAI-enrichment_score_table.txt"))
adaptive_enrichments_list[["moderate"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/adaptive_spliceai_moderate/adaptive_spliceai_moderate_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_SpliceAI-enrichment_score_table.txt"))
adaptive_enrichments_list[["strong"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/adaptive_spliceai_strong/adaptive_spliceai_strong_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_SpliceAI-enrichment_score_table.txt"))

archaic_enrichments_list <- list()
archaic_enrichments_list[["weak"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/introgressed_spliceai_weak/introgressed_spliceai_weak_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_SpliceAI-enrichment_score_table.txt"))
archaic_enrichments_list[["moderate"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/introgressed_spliceai_moderate/introgressed_spliceai_moderate_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_SpliceAI-enrichment_score_table.txt"))
archaic_enrichments_list[["strong"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/introgressed_spliceai_strong/introgressed_spliceai_strong_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_SpliceAI-enrichment_score_table.txt"))

# join files
adaptive_enrichments_final <- bind_rows(adaptive_enrichments_list, .id="region")
adaptive_enrichments_final$region <- factor(adaptive_enrichments_final$region, levels=c("weak", "moderate", "strong"))
write_tsv(adaptive_enrichments_final, gzfile("../../results/enrichment_GTEx_QTLs_v2/direct_compare/adaptive_spliceai-region-recmap_access_enrichment_scores_table_pivot_LDtagN_pointerror_direct.txt.gz"))

archaic_enrichments_final <- bind_rows(archaic_enrichments_list, .id="region")
archaic_enrichments_final$region <- factor(archaic_enrichments_final$region, levels=c("weak", "moderate", "strong"))
write_tsv(archaic_enrichments_final, gzfile("../../results/enrichment_GTEx_QTLs_v2/direct_compare/introgressed_spliceai-region-recmap_access_enrichment_scores_table_pivot_LDtagN_pointerror_direct.txt.gz"))
...
ggboxplot(enrichments_final %>% filter(region=="weak"), 
	...
	xlab("") + ylab("log2 odds-ratio") + theme(plot.margin = margin(0,0,0,1, "in")) + ylim(c(-2, 3.5))
ggsave("../../results/enrichment_GTEx_QTLs_v2/direct_compare/direct_compare_recmap_access_AF_LDtagN_adaptive_vs_introgressed_SpliceAI_weak.pdf", device=cairo_pdf, scale=0.65)
...
ggboxplot(enrichments_final %>% filter(region=="moderate"), 
	...
	theme(legend.position = "none") + theme(aspect.ratio=1.5) + theme(plot.background=element_blank()) + 
	xlab("") + ylab("log2 odds-ratio") + theme(plot.margin = margin(0,0,0,1, "in")) + ylim(c(-2, 3.5))
ggsave("../../results/enrichment_GTEx_QTLs_v2/direct_compare/direct_compare_recmap_access_AF_LDtagN_adaptive_vs_introgressed_SpliceAI_moderate.pdf", device=cairo_pdf, scale=0.65)
...
ggboxplot(enrichments_final %>% filter(region=="strong"), 
	...
	theme(legend.position = "none") + theme(aspect.ratio=1.5) + theme(plot.background=element_blank()) + 
	xlab("") + ylab("log2 odds-ratio") + theme(plot.margin = margin(0,0,0,1, "in")) + ylim(c(-2, 3.5))
ggsave("../../results/enrichment_GTEx_QTLs_v2/direct_compare/direct_compare_recmap_access_AF_LDtagN_adaptive_vs_introgressed_SpliceAI_strong.pdf", device=cairo_pdf, scale=0.65)
