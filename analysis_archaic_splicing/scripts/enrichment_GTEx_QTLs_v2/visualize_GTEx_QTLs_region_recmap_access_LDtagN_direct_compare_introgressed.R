...
adaptive_enrichments_list[["eQTL"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/adaptively_introgressed/adaptively_introgressed_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_eQTLs_NULL-enrichment_score_table.txt"))
adaptive_enrichments_list[["sQTL"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/adaptively_introgressed/adaptively_introgressed_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_sQTLs_NULL-enrichment_score_table.txt"))

archaic_enrichments_list <- list()
archaic_enrichments_list[["eQTL"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/archaic_introgressed/archaic_introgressed_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_eQTLs_NULL-enrichment_score_table.txt"))
archaic_enrichments_list[["sQTL"]] <- as_tibble(fread("../../results/enrichment_GTEx_QTLs_v2/archaic_introgressed/archaic_introgressed_matched_EUR_AC_binxEUR_LDtagN_bin_control_recmap_access_sQTLs_NULL-enrichment_score_table.txt"))
...
write_tsv(adaptive_enrichments_final, gzfile("../../results/enrichment_GTEx_QTLs_v2/direct_compare/adaptively_introgressed-region-recmap_access_enrichment_scores_table_pivot_LDtagN_pointerror_direct.txt.gz"))
...
write_tsv(archaic_enrichments_final, gzfile("../../results/enrichment_GTEx_QTLs_v2/direct_compare/archaic_introgressed-region-recmap_access_enrichment_scores_table_pivot_LDtagN_pointerror_direct.txt.gz"))
...
ggsave("../../results/enrichment_GTEx_QTLs_v2/direct_compare/direct_compare_recmap_access_AF_LDtagN_adaptive_vs_introgressed_eQTL.pdf", device=cairo_pdf, scale=0.65)
...
ggboxplot(enrichments_final %>% filter(region=="sQTL"), 
	...
	theme(legend.position = "none") + theme(aspect.ratio=1.5) + theme(plot.background=element_blank()) + 
	xlab("") + ylab("log2 odds-ratio") + theme(plot.margin = margin(0,0,0,1, "in")) + ylim(c(-0.5, 1.5))
ggsave("../../results/enrichment_GTEx_QTLs_v2/direct_compare/direct_compare_recmap_access_AF_LDtagN_adaptive_vs_introgressed_sQTL.pdf", device=cairo_pdf, scale=0.65)
