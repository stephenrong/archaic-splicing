...
# collated_AF_LD_AI_NAI_hub <- collated_AF_LD_AI_NAI_hub %>% 
# 	filter(EUR_LDtagN > 0)
...
ggplot(collated_AF_LD_AI_NAI_SFS, aes(EUR_LDtagN, EUR_LDtagN_prop, color=Description)) + 
	geom_line() + 
	theme_base() + theme(aspect.ratio=0.5) + 
...
ggsave("../../results/enrichment_GTEx_QTLs_v2/figures/collated_AF_LD_AI_NAI_hub_sfs_EUR_LDtagN_v2.pdf")

ggplot(collated_AF_LD_AI_NAI_hub, aes(EUR_LDtagN, color=Description)) + 
	stat_ecdf() + 
	theme_base() + theme(aspect.ratio=0.5) + 
	xlab("SNPs in LD (<1Mb, MAF>0.01, R^2>0.2)") + ylab("Empirical CDF") + 
	theme(plot.background=element_blank()) + scale_color_met_d("Juarez") + scale_x_log10()
ggsave("../../results/enrichment_GTEx_QTLs_v2/figures/collated_AF_LD_AI_NAI_hub_ecdf_EUR_LDtagN_v2.pdf")
