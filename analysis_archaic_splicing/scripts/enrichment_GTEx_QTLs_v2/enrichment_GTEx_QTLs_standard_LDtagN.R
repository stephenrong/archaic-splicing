...
option_list = list(
	...
	make_option(
		c("-x", "--background_index"), type="character", default="../../results/enrichment_GTEx_QTLs_v2/ALL_1KGP_phase3_MAF0.01_hapR2_notAI_hub_EUR_AC_binxEUR_LDtagN_index.rds", help="index of binned rows in background file", metavar="character"), 
	make_option(
...
filter_input_file <- load_input_file %>% 
	dplyr::select(hub_variant_ID, EUR_AC_bin, EUR_LDtagN_bin)
