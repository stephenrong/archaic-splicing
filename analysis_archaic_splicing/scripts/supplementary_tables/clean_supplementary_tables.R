...
# mapsy results
supplement_mapsy_results <- as_tibble(fread("../../results/mapsy_to_variant_table_updated/Neanderthal_updated_mapsy_to_variant_table_no_QC_filter.txt.gz"))
supplement_mapsy_results_final <- supplement_mapsy_results %>% 
	dplyr::select(source,construct_type,hub_variant_ID,hub_reference_genome,hub_variant_CHROM,hub_variant_POS,hub_variant_REF,hub_variant_ALT,hub_variant_ANC,hub_variant_DER,hub_variant_CASE,exon_seqnames,exon_start,exon_end,exon_width,exon_strand,exon_exon_id,exon_exon_name,exon_exon_rank,exon_exon_internal,exon_gene_id,exon_gene_type,exon_gene_name,exon_transcript_id,exon_transcript_type,exon_transcript_name,Common_id_three,Common_id_five,Common_five,Common_three,Sub_library,Order_assign,Primer_pair_new,Primer_5_prime,Primer_3_prime,Order_final_id_wt,Order_final_id_mt,Order_debug_sequence_wt,Order_debug_sequence_mt,Order_final_sequence_wt,Order_final_sequence_mt,Order_length_wt,Order_length_mt,ref_input_readc_a_wt,ref_input_readc_b_wt,ref_input_readc_c_wt,ref_input_readc_a_mt,ref_input_readc_b_mt,ref_input_readc_c_mt,ref_output_readc_a_wt,ref_output_readc_b_wt,ref_output_readc_c_wt,ref_output_readc_a_mt,ref_output_readc_b_mt,ref_output_readc_c_mt,filter_input_readcs,filter_output_readcs,mpralm.logFC,mpralm.CI.L,mpralm.CI.R,mpralm.t,mpralm.P.Value,mpralm.adj.Pval,mpralm.sigvar,mpralm.sigclass,mpralm.ANCDER.logFC) %>% 
	dplyr::rename(input_read_count_rep_a_wt=ref_input_readc_a_wt, input_read_count_rep_b_wt=ref_input_readc_b_wt, input_read_count_rep_c_wt=ref_input_readc_c_wt, input_read_count_rep_a_mt=ref_input_readc_a_mt, input_read_count_rep_b_mt=ref_input_readc_b_mt, input_read_count_rep_c_mt=ref_input_readc_c_mt, output_read_count_rep_a_wt=ref_output_readc_a_wt, output_read_count_rep_b_wt=ref_output_readc_b_wt, output_read_count_rep_c_wt=ref_output_readc_c_wt, output_read_count_rep_a_mt=ref_output_readc_a_mt, output_read_count_rep_b_mt=ref_output_readc_b_mt, output_read_count_rep_c_mt=ref_output_readc_c_mt) %>% 
	...
	dplyr::rename(QC_filter_input_read_count=filter_input_readcs, QC_filter_output_read_count=filter_output_readcs)
...
supplement_annotations_vep_final <- supplement_annotations_vep %>% 
	dplyr::select(hub_variant_ID,exon_gene_name,VEP) %>% 
	dplyr::rename(main_variant_ID=hub_variant_ID,VEP_consequence=VEP)
supplement_annotations_vep_final <- supplement_mapsy_results_final %>% 
...
supplement_annotations_spliceai_final <- supplement_annotations_spliceai %>% 
	dplyr::select(hub_variant_ID,exon_gene_name,ALLELE,SYMBOL,DS_AG,DS_AL,DS_DG,DS_DL,DP_AG,DP_AL,DP_DG,DP_DL,spliceai_max) %>% 
...
supplement_mapsy_results_merged_temp <- supplement_mapsy_results_merged %>% 
	dplyr::select(contains("_read_count_"))
supplement_mapsy_results_merged_summ <- 
	bind_cols(
		enframe(apply(supplement_mapsy_results_merged_temp, 2, function(x) {nrow(supplement_mapsy_results_merged_temp)}), name="Library", value="Sequences"), 
		enframe(apply(supplement_mapsy_results_merged_temp, 2, function(x) {quantile(x, 0)}), name="Library", value="Min read count")[,2], 
		enframe(apply(supplement_mapsy_results_merged_temp, 2, function(x) {quantile(x, 0.25)}), name="Library", value="First quartile read count")[,2], 
		enframe(apply(supplement_mapsy_results_merged_temp, 2, function(x) {quantile(x, 0.50)}), name="Library", value="Median read count")[,2], 
		enframe(apply(supplement_mapsy_results_merged_temp, 2, function(x) {quantile(x, 0.75)}), name="Library", value="Third quantile read count")[,2], 
		enframe(apply(supplement_mapsy_results_merged_temp, 2, function(x) {quantile(x, 1)}), name="Library", value="Max read count")[,2], 
		enframe(apply(supplement_mapsy_results_merged_temp, 2, mean), name="Library", value="Mean read count")[,2],
		enframe(apply(supplement_mapsy_results_merged_temp, 2, sum), name="Library", value="Total mapped reads")[,2]
	)
supplement_mapsy_results_merged_summ$Library <- c("input rep a (wt)", "input rep b (wt)", "input rep c (wt)", "input rep a (mt)", "input rep b (mt)", "input rep c (mt)", "output rep a (wt)", "output rep b (wt)", "output rep c (wt)", "output rep a (mt)", "output rep b (mt)", "output rep c (mt)")
write_tsv(supplement_mapsy_results_merged_summ, gzfile("../../results/supplementary_tables/Rong_et_al_Supplementary_Data_SX_MaPSy_read_count_coverage_statistics.txt.gz"))
...
supplement_1KGP_final <- supplement_1KGP %>% 
	dplyr::select(hub_reference_genome,hub_variant_CHROM,hub_variant_POS,hub_variant_ID,hub_variant_REF,hub_variant_ALT,hub_variant_ANC,hub_variant_DER,hub_variant_CASE,CHROM,POS,ID,REF,ALT,QUAL,FILTER,AC,AF,AN,NS,DP,EAS_AF,AMR_AF,AFR_AF,EUR_AF,SAS_AF,EAS_AC,AMR_AC,AFR_AC,EUR_AC,SAS_AC,EUR_AC_bin,EUR_LDtagN,EUR_LDtagN_bin) %>% 
	dplyr::rename(main_reference_genome=hub_reference_genome,main_variant_CHROM=hub_variant_CHROM,main_variant_POS=hub_variant_POS,main_variant_ID=hub_variant_ID,main_variant_REF=hub_variant_REF,main_variant_ALT=hub_variant_ALT,main_variant_ANC=hub_variant_ANC,main_variant_DER=hub_variant_DER,main_variant_CASE=hub_variant_CASE,`1KGP_CHROM`=CHROM,`1KGP_POS`=POS,`1KGP_ID`=ID,`1KGP_REF`=REF,`1KGP_ALT`=ALT,`1KGP_QUAL`=QUAL,`1KGP_FILTER`=FILTER,`1KGP_AC`=AC,`1KGP_AF`=AF,`1KGP_AN`=AN,`1KGP_NS`=NS,`1KGP_DP`=DP,`1KGP_EAS_AF`=EAS_AF,`1KGP_AMR_AF`=AMR_AF,`1KGP_AFR_AF`=AFR_AF,`1KGP_EUR_AF`=EUR_AF,`1KGP_SAS_AF`=SAS_AF,`1KGP_EAS_AC`=EAS_AC,`1KGP_AMR_AC`=AMR_AC,`1KGP_AFR_AC`=AFR_AC,`1KGP_EUR_AC`=EUR_AC,`1KGP_SAS_AC`=SAS_AC,`1KGP_EUR_AC_bin`=EUR_AC_bin,`1KGP_EUR_LD`=EUR_LDtagN,`1KGP_EUR_LD_bin`=EUR_LDtagN_bin)
...
supplement_1KGP_spliceai <- as_tibble(fread("../../results/annotate_splice_prediction/ALL_1KGP_phase3_MAF0.01_hapR2_hub_spliceai_gs_raw_dedup.txt.gz"))
