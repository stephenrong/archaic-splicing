...
merge_variants_B_stat_mask_1KGP_archaic_gnomAD_input <- read.vcfR(merge_variants_B_stat_mask_1KGP_archaic_gnomAD_file) %>% 
...
for (col in col_list) {
	let(c(VAR=col), 
		merge_variants_B_stat_mask_1KGP_archaic_gnomAD_input <- merge_variants_B_stat_mask_1KGP_archaic_gnomAD_input %>% 
			mutate(VAR = as.numeric(gsub(".*=", "", VAR))))
}
names(merge_variants_B_stat_mask_1KGP_archaic_gnomAD_input) <- 
...
merge_variants_B_stat_mask_1KGP_archaic_gnomAD_hub <- merge_variants_B_stat_mask_1KGP_archaic_hub %>% 
	left_join(merge_variants_B_stat_mask_1KGP_archaic_gnomAD_input)

# 	save file
print("gnomAD save file")
write_tsv(merge_variants_B_stat_mask_1KGP_archaic_gnomAD_hub, gzfile("../../results/preprocess_1KGP_SNPs/merge_variants_B_stat_mask_1KGP_archaic_gnomAD_hub.txt.gz"))

# 	check duplicates
print(table(table(merge_variants_B_stat_mask_1KGP_archaic_gnomAD_hub$hub_variant_ID)))
