...
hg19_REFisDER_1KGP_archaic_gnomAD_file <- "../../results/preprocess_1KGP_SNPs/hg19_REFisDER_1KGP_archaic_gnomAD_file.vcf.gz"
hg19_REFisDER_1KGP_archaic_gnomAD_input <- read.vcfR(hg19_REFisDER_1KGP_archaic_gnomAD_file) %>% 
...
for (col in col_list) {
	let(c(VAR=col), 
		hg19_REFisDER_1KGP_archaic_gnomAD_input <- hg19_REFisDER_1KGP_archaic_gnomAD_input %>% 
			mutate(VAR = as.numeric(gsub(".*=", "", VAR))))
}
names(hg19_REFisDER_1KGP_archaic_gnomAD_input) <- 
...
hg19_REFisDER_1KGP_archaic_gnomAD_hub <- hg19_REFisDER_1KGP_archaic_hub %>% 
	left_join(hg19_REFisDER_1KGP_archaic_gnomAD_input)

# 	save file
print("gnomAD save file")
write_tsv(hg19_REFisDER_1KGP_archaic_gnomAD_hub, gzfile("../../results/preprocess_1KGP_SNPs/hg19_REFisDER_1KGP_archaic_gnomAD_hub.txt.gz"))

# 	check duplicates
print(table(table(hg19_REFisDER_1KGP_archaic_gnomAD_hub$hub_variant_ID)))
