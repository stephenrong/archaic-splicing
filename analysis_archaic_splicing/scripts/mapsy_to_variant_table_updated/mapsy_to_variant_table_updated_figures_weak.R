...
mapsy_proportion <- function(table, column) {
	table_prop <- table[which(table[[column]]),] %>% 
		summarise(
			mapsy_sig = sum((mpralm.sigvar)&(abs(mpralm.ANCDER.logFC)>log2(1.5))&(abs(mpralm.ANCDER.logFC)<log2(3)), na.rm=T), 
			mapsy_nonsig = n()-sum((mpralm.sigvar)&(abs(mpralm.ANCDER.logFC)>log2(1.5))&(abs(mpralm.ANCDER.logFC)<log2(3)), na.rm=T), 
			mapsy_total = n()
		)
...
enrichment_mapsy_figures <- function(list_cols, list_names, output_file, spacer, width) {
	...
	if (min(stat_test_pair$p) < 0.1) {
		...
		p <- ggbarplot(mapsy_proportion_tab, 
			x = "description", y = "mapsy_prop", fill="#92C5DE") + 
			...
			theme(legend.position = "none") + ylab("Proportion MaPSy weak ESM") + 
...
output_file <- "../../results/mapsy_to_variant_table_updated/Neanderthal_updated_mapsy_prop_bar_plot_stat_weak-lineage_specific_nodeni.pdf"
spacer <- 0.05
scale <- 0.55
width <- 0.2
...
output_file <- "../../results/mapsy_to_variant_table_updated/Neanderthal_updated_mapsy_prop_bar_plot_stat_weak-lineage_specific_nodeni-fixed.pdf"
spacer <- 0.05
scale <- 0.65  # 0.55
width <- 0.2
enrichment_mapsy_figures(list_cols, list_names, output_file, spacer, width)
...
output_file <- "../../results/mapsy_to_variant_table_updated/Neanderthal_updated_mapsy_prop_bar_plot_stat_weak-introgressed.pdf"
spacer <- 0.05
scale <- 0.65
width <- 0.2
enrichment_mapsy_figures(list_cols, list_names, output_file, spacer, width)
