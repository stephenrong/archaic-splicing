...
enrichment_mapsy_figures <- function(list_cols, list_names, output_file, spacer, width) {
	...
	if (min(stat_test_pair$p) < 0.1) {
		...
		p <- ggbarplot(mapsy_proportion_tab, 
			...
			theme(legend.position = "none") + ylab("Proportion MaPSy strong pos ESM") + 
...
output_file <- "../../results/mapsy_to_variant_table_updated/Neanderthal_updated_mapsy_prop_bar_plot_stat_strong_pos-introgressed.pdf"
spacer <- 0.01
scale <- 0.65
width <- 0.2
enrichment_mapsy_figures(list_cols, list_names, output_file, spacer, width)
