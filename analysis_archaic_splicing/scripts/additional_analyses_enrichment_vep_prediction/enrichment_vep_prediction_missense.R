...
final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_hub_vep <- final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_hub_vep %>% 
	mutate(vep_missense_sig = (missense_variant))

# get proportion
vep_missense_proportion <- function(table, column) {
	table_prop <- table[which(table[[column]]),] %>% 
		filter(!is.na(vep_missense_sig)) %>% 
		summarise(
			vep_missense_sig = sum(vep_missense_sig), 
			vep_missense_nonsig = n()-sum(vep_missense_sig), 
			vep_missense_total = n()
		)
	table_prop <- table_prop %>% 
		mutate(vep_missense_prop = binconf(vep_missense_sig, vep_missense_total)[1]) %>% 
		mutate(vep_missense_propLCI = binconf(vep_missense_sig, vep_missense_total)[2]) %>% 
		mutate(vep_missense_propUCI = binconf(vep_missense_sig, vep_missense_total)[3])
...
enrichment_vep_missense_figures <- function(list_cols, list_names, output_file, spacer, width) {
	# get proportions
	vep_missense_proportion_tab <- NULL
	for (col in list_cols){
		temp <- vep_missense_proportion(final_v2_variants_B_stat_mask_1KGP_archaic_gnomAD_hub_vep, col)
		if (is.null(vep_missense_proportion_tab)) {
			vep_missense_proportion_tab <- temp
		} else {
			vep_missense_proportion_tab <- bind_rows(vep_missense_proportion_tab, temp)
		}
	}
	...
	if (min(stat_test_pair$p) < 0.1) {
		...
		p <- ggbarplot(vep_missense_proportion_tab %>% arrange(vep_missense_prop), 
			x = "description", y = "vep_missense_prop", fill="#2ca25f") + 
			geom_errorbar(aes(description, 
				ymin=vep_missense_propLCI, 
				ymax=vep_missense_propUCI), stat="identity", width=width) + 
			stat_pvalue_manual(stat_test_pair_sig, label = "p", 
				tip.length = 0.01, label.size=3) + 
			theme(legend.position = "none") + ylab("Proportion missense variant") + 
...
output_file <- "../../results/additional_analyses_enrichment_vep_prediction/Neanderthal_updated_vep_missense_prop_bar_plot_stat_missense-lineage_specific_nodeni.pdf"
spacer <- 0.0005
scale <- 0.55
width <- 0.2
enrichment_vep_missense_figures(list_cols, list_names, output_file, spacer, width)
